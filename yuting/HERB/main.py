...
from dataloader_new import build_loader
...
def set_environment(args, tlogger):
    ...
    return model, optimizer, scaler, amp_context, start_epoch
    ...
    for fold in range(5):
        ...
        if args.use_wandb:
            wandb.init(entity=args.wandb_entity,
                    ...
                    name=args.exp_name,
                    config=args)
            wandb.run.summary[f"best_{fold+1}_acc"] = best_acc
            wandb.run.summary[f"best_{fold+1}_eval_name"] = best_eval_name
            wandb.run.summary[f"best_{fold+1}_epoch"] = 0
        print(f'Fold {fold+1}---------------')
        model, optimizer, scaler, amp_context, start_epoch = set_environment(args, tlogger)

        ### = = = =  Dataset and Data Loader = = = =  
        tlogger.print("Building Dataloader....")
        
        train_loader, val_loader = build_loader(fold)
        ...
        for epoch in range(start_epoch, args.max_epochs):
            ...
            if True:
                ...
                acc = -1
                if val_loader is not None:
                    tlogger.print("Start Evaluating {} Fold, {} Epoch".format(fold + 1, epoch + 1))
                ...
                if acc > best_acc:
                    ...
                    torch.save(checkpoint, args.save_dir + "backup/fold_" + str(fold+1) + "_best.pt")
                if args.use_wandb:
